...
function command(command_to_send, capture_start_tag, capture_end_tag, regexp, silent, omit, no_prompt_after, call_with_result)
   ...
   print("compact", compact_mode, "prompt", prompt_mode)
   ...
   -- -- if (not no_prompt_after) and (prompt_mode == "YES") then
   -- --    if compact_mode == "NO" then
   ...
   -- -- if no_prompt_after and (prompt_mode == "YES") then
   -- --    Send_GMCP_Packet("config prompt YES")
   -- -- end

   -- if (compact_mode == "NO") and not ((not no_prompt_after) and (prompt_mode == "YES")) then
   --    Send_GMCP_Packet("config compact NO")
   -- end
